              ...
              <Col xs={24} lg={12}>
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden mt-3 shadow-sm">
                  {Array.isArray(insight.reasons) && insight.reasons.length > 0 ? (
                    insight.reasons.map((item: string, idx: number) => (
                      <div key={idx} className="p-3 border-b border-slate-200 last:border-b-0 bg-white text-slate-700 text-sm">
                        • {item}
                      </div>
                    ))
                  ) : (
                    <div className="p-4 text-center text-slate-500 text-sm">لا توجد بيانات</div>
                  )}
                </div>
              </Col>
              ...
              <Col xs={24} lg={12}>
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden mt-3 shadow-sm">
                  {Array.isArray(insight.recommendations) && insight.recommendations.length > 0 ? (
                    insight.recommendations.map((item: string, idx: number) => (
                      <div key={idx} className="p-3 border-b border-slate-200 last:border-b-0 bg-white text-slate-700 text-sm">
                        • {item}
                      </div>
                    ))
